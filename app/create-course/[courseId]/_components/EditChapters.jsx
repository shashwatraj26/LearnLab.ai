...
function EditChapters({course,index,refreshData}) {
  ...
  const onUpdateHandler=async()=>{
    ...
    const result=await db.update(CourseList).set({courseOutput:course?.courseOutput}).where(eq(CourseList?.id,course?.id)).returning({id:CourseList.id});
    refreshData(true);
  }
...
<Dialog>
  ...
  <DialogContent>
    <DialogHeader>
      <DialogTitle>Edit this Chapter</DialogTitle>
      <DialogDescription>
      <div className='mt-3'>
